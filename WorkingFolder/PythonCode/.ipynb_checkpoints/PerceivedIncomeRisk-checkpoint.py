...
#
# I also recognize what is really relevant to many economic decisions such as consumption, real income instead of nominal income is truely relevant. Thanks to the availability of inflation expectation and inflation uncertainty (also estimated from density question) can be used to convert nominal earning growth moments to real terms. In particular, the real earning growth rate is expected nominal growth minus inflation expectation. 
#
#
# \begin{eqnarray}
...
#
#
# ##  [Cross-sectional heterogeneity](MacroRiskProfile.ipynb)
#
# This section inspects some basic cross-sectional patterns of the subject moments of labor income. In the figure below, I plot the histograms of $\overline\Delta y_{i,t}$, $\overline{var}_{i,t}$,$\overline{iqr}_{i,t}$ and $\overline {skw}_{i,t}$, $\overline {\Delta y^{r}}_{i,t}$, $\overline{var^{r}}_{i,t}$. 
#
...
# ##  [Correlation with asset returns](MacroRiskProfile.ipynb)
#
...
#
# With these considerations, in the figure below, I plot the median perceived risk and skewness of both nominal and real earning along with the contemporaneous stock market returns by the end of each month (also true for the mean, see appendix). In order to account for the fact that the survey is undertaken in the middle of the month while the return is computed at the end of the month, I take the lag the income moments by 1 or 2 months when calculating the correlation coefficient. Although a Pearson test of the correlation coefficients is only significant for a 2-month lag, overall, the income risks measured by variance and IQR for both nominal and real earning post a negative correlation with the realized stock return a few months ahead. The subjective skewness has also a negative associated with the realized stock return in the near future. 
#
# [figures of stock market return and income risks](../Graphs/pop/ts.jpg)
#
# [table on correlation coefficient of different lags](../Tables/latex/macro_corr.tex)
#
...
# + {"caption": "Perceived Income by Group", "code_folding": [12], "label": "fig:boxplotbygroup", "widefigure": true}
## insert figures 
graph_path = os.path.join(path,'../Graphs/ind/')

fig_list = ['boxplot_HHinc.jpg',
            'boxplot_educ.jpg',
            'boxplot_gender.jpg']
