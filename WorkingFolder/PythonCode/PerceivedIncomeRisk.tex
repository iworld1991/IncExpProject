...
\makeatother
    ...
    \hypertarget{perceived-income-risks}{%
\section{Perceived Income Risks}\label{perceived-income-risks}}
...
`'The devil is the in higher moments.'' Even if two people share
identical expected income and homogeneous preferences, different degrees
of income risks still lead to starkly different decisions such as
...
\cite{7250895/FHA8RDSF} as an example.) But one assumption implicitly
made when doing this is that the agents in the model perfectly
understand thus agree on the income risk profile imposed on them. As
shown by the actively developing literature on expectation formation, in
particular, the mounting evidence on heterogeneity in economic
expectations held by micro agents, this assumption seems be too
...
\subsubsection{Relevant Literature and Potential
...
literature: `insurance or information'. \cite{7250895/CJ63N699},
\cite{7250895/TH74LHW4} In any empirical tests of consumption insurance
or consumption response to income shocks, there is always a worry that
what is interpreted as shock has actually already entered in the agents'
information set or exactly the opposite. For instance, since
econometricians have no full access to what agents truely know in their
information set, what is interpreted as excessive sensitivity may be
simply because agents do not have access to the recently realized
shocks. My paper shares a similar spirit with the few papers above in
the sense that we try to tackle the identification problem in the same
approach: directly using the expectation data and explicitly controling
what are truely conditional expectations of the agents making the
...
these few papers is that I focus on higher moments, i.e.~income risks
and skewness by utilizing the recently available density forecasts of
labor income. Part of my exercise can be also seen as an extension of
the New York Fed
...
\cite{7250895/5XVTCNHG} found that households' expectation of income
exhibits an over-persistent bias using both expected and realized
household income from Michigan household survey. The paper also shows
that incorporating such bias affects the aggregate consumption function
by distorting the cross-sectional distributions of marginal propensity
to consume(MPCs) across the population. \cite{7250895/NENWDCHR}
reconciles the low micro-MPC and high macro-MPCs by introducing to the
model an information rigidity of households in learning about macro news
while being updated about micro news. (Lian 2019) shows that an
imperfect perception of wealth accounts such phenomenon as excess
sensitivity to current income and higher MPCs out of wealth than current
income and so forth. \_\textbf{My paper has a similar flavor to all of
these works by exploring the behavioral implications of households'
perceptive imperfection. But unlike their work, my paper dominantly
focues on the implications of heterogeneity in perceived higher moments
such as risks and skewness.}
...
in economic surveys (\cite{undefined}, \cite{7250895/72CMBZAS},
\cite{7250895/NYPWMNU7}). Although the initial suspicion concerning to
people's ability in understanding, using and answering probabilistic
questions is understandable, Bertrand and \cite{7250895/5XEYZFGT} and
other works have shown respondents have the consistent ability and
willingness to assign a probability (or ``percent chance'') to future
events. \cite{7250895/8PVZ2ADL} have a thorough discussion on designing,
experimenting and implementing the consumer expectation surveys to
ensure the quality of the responses2. Broadly speaking, the advocators
have argued that going beyond the revealed preference approach,
availability to survey data provides economists with direct information
on agents' expectations and helps avoids imposing arbitrary assumptions.
This insight holds for not only point forecast but also and even more
importantly, for uncertainty, because for any economic decision made by
a risk-averse agent, not only the expectation but also the perceived
risks matter a great deal.

\hypertarget{a-broader-motivation}{%
\subsubsection{A broader motivation}\label{a-broader-motivation}}
...
\subsection{Data, Variables and Density
Estimation}\label{data-variables-and-density-estimation}}

The data used for this paper is from the core module of Survey of
Consumer Expectation(SCE) conducted by New York Fed, a monthly online
survey for a rotating panel of around 1,300 household heads. The sample
period in my paper spans from June 2013 to June 2018, in total 60
months. This makes about 79000 household-year observations, among which
around 53,000 observations provide non-empty answers to the density
question on earning growth.

In particular, the questionare asks each respondent to fill perceived
probabilities of their same-job-hour earning growth to pre-defined
non-overlapping bins. The question is framed as below:

\begin{verbatim}
"Suppose that 12 months from now, you are working in the exact same [“main” if Q11>1] job at the same place you currently work, and working the exact same number of hours. In your view, what would you say is the percent chance that 12 months from now: increased by x% or more?"
\end{verbatim}

As a special feature of online questinare, the survey moves on to next
question only if the probabilities filled in all bins add up to one.
This ensures the basic probabilistic consistency of the answers crutial
for any further analysis using this data. Besides, the earning growth
...
requirements help make sure the comparability of the answers across
time, and also excludes the potential changes in earnings driven by
endogeous labor supply decisions, i.e.~working for more. Second, the
earning expectations, risks and tail risks measured here is only
conditional. It excludes either unemployment, i.e.~zero earning, or an
upward movement in the job ladder, i.e.~a different earning growth rate.
Therefore, it does not fully reflect the labor income risk profile
relevant for each individual.

To the extent that we want to tease out the voluntary decision from the
exogeous labor income, only the exclusion of unemployment scenario is
relevant for my purpose in characterizing the labor income risks. This
is assuring because if so, we could interprete the moments of
same-job-hour earning growth as an upper bound for the level of growth
rate and a lower bound for the income risks. To put it another way, the
expected earning growth conditional on employment is higher than the
unditional one, and the conditional earning risk is lower than the
unconditional one. At the same time, since SCE seperately elcits the
perceived probability of losing the curent job for each respondent, I
could adjust the measured labor income moments taking into account the
unemployment risk.

With the histogram answers for each individual in hand, I follow
\cite{7250895/2JGDME82} to fit each of them with a parametric
...
The reasons why I discuss the exact procedures for density distribution
is not symbolic. It is important for this paper's purpose because I need
to make sure the estimation assumptions of the distribution does not
mechanically distort my distribution of the moments.
\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#

Since the micro data provided in SCE website already includes the
estimated mean, variance and IQR by the staff economists following the
exactly same approach, I directly use their estimates for these moments.
...
For all the moments estimates, there are inevitablly extreme values.
This could be due to the idiosyncratic answers provided by the original
respodnent, or some non-convergence of the numeric estimation program.
...
\tightlist
\item
  conversion from nominal to real. expected real earning growth =
  expected nominal earning growth - inflation var(expected earning
  growth) = var(expected nominal earning growth) - var(inflation)
\item
  ...
  \tightlist
  \item
    this posted a lower bound for the risk as the question only askes
    about the earning growth for the same job.
  \item
    nead to adjust by the unemployment risks
  \end{itemize}
\end{itemize}

    \hypertarget{subjective-income-risks-and-macro-environment}{%
\subsection{Subjective income risks and macro
environment}\label{subjective-income-risks-and-macro-environment}}

\hypertarget{cross-sectional-distribution-of-income-moments}{%
\subsubsection{\texorpdfstring{\href{MacroRiskProfile.ipynb}{Cross-sectional
distribution of income
moments}}{Cross-sectional distribution of income moments}}\label{cross-sectional-distribution-of-income-moments}}
...
\includegraphics{attachment:hist_Q24_mean.jpg}
\includegraphics{attachment:hist_Q24_var.jpg}
\includegraphics{attachment:histIncSkew.jpg}
\includegraphics{attachment:hist_Q24_rmean.jpg}
\includegraphics{attachment:hist_Q24_rvar.jpg}

\hypertarget{correlation-with-asset-returns}{%
\subsubsection{\texorpdfstring{\href{MacroRiskProfile.ipynb}{Correlation
with asset
...
\cite{7250895/7ZA2CX5N}, \cite{undefined}, . Variables such as
consumption and earning are interval measures, reported as an average
over a period, while the stock return is a spot measure computed between
two points of the time. As a result, if the unit of the time for the
underlying income process is at a higher frequency than the measured
interval (an extreme case being the continuous-time), the measured
variable will exhibit upward biased autocorrelation and correlation with
other underlying random walk series in the same frequency. In my
context, such a problem can be partly mitigated by the availability of
monthly frequency of earning expectations, if we assume the unit of time
of the underlying stochastic process is a month. Then the directly
observed monthly correlation of the two cannot be driven by the time
aggregation problem. What also becomes immediately clear from this
considration is that I should not examine the correlation of the two
series in moving average terms, because it will cause the time
aggregation problem. This point will be discussed in greater detail in
the next section when I decompose the perceived income risks to
...
\includegraphics{attachment:tsMed3mvmean.jpg}
\includegraphics{attachment:tsMed3mvvar.jpg}
\includegraphics{attachment:tsEstMean3mvskew.jpg}

\hypertarget{individual-characteristics-and-perceived-income-risks}{%
\subsubsection{\texorpdfstring{\href{MicroRiskProfile.ipynb}{Individual
characteristics and perceived income
risks}}{Individual characteristics and perceived income risks}}\label{individual-characteristics-and-perceived-income-risks}}
...
dependent variable \(\textit{IncMom}_{i,t}\) represents alternative
measures of perceived income risks for either nominal or real earning.
...
\hypertarget{perceived-income-risks-and-decision}{%
\subsubsection{Perceived income risks and
decision}\label{perceived-income-risks-and-decision}}
...
\end{itemize}
    ...
    \hypertarget{decomposing-perceived-income-risks}{%
\subsection{Decomposing perceived income
risks}\label{decomposing-perceived-income-risks}}

\hypertarget{an-illustration-of-the-idea-in-an-permanent-transitory-income-process}{%
\subsubsection{An illustration of the idea in an permanent-transitory
income
process}\label{an-illustration-of-the-idea-in-an-permanent-transitory-income-process}}
...
\subsubsection{Other moments from rational
...
\end{itemize}
    ...
    \hypertarget{a-simple-example-with-half-year-surveys-of-one-year-ahead-earning-growth}{%
\subsubsection{A Simple Example with Half-year Surveys of One-year-ahead
Earning
Growth}\label{a-simple-example-with-half-year-surveys-of-one-year-ahead-earning-growth}}
...
\hypertarget{alternative-income-process}{%
\subsubsection{Alternative income
process}\label{alternative-income-process}}

    \hypertarget{sketch-of-the-model}{%
\subsection{Sketch of the Model}\label{sketch-of-the-model}}

\hypertarget{new-ingredients-of-the-life-cycle-model}{%
\subsubsection{New ingredients of the life-cycle
model}\label{new-ingredients-of-the-life-cycle-model}}

\hypertarget{anticipated-insights}{%
\subsubsection{Anticipated insights}\label{anticipated-insights}}
